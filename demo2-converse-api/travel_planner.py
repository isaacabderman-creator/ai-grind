...
SYSTEM_PROMPT = """\
You're a helpful travel guide that helps users plan visits to city. 
Never answer from memory just. If you are unsure, call a tool or return you don't know.
Only use tool results for recommendations.
Output format: 
 - Use Morning, Afternoon, Evening headers
 - Be concise and keep the response under 150 words\
"""
            ...
            "inputSchema": {
                "json": {
                    ...
                    "properties": {
                        ...
                        "city": {
                            "type": "string",
                            "description": "The name of the city to get attractions of."
                        },
...
def get_weather(city: str, date: str) -> dict:
    ...
    key = (city.lower(), date)
    error = {"city": city, "date": date, "condition": "No data available"}

    return WEATHER_DATA.get(key, error)
...
def get_top_attractions(city: str) -> dict:
    ...
    key = city.lower()
    error = { "city": city, "attractions": []}
    return ATTRACTIONS_DATA.get(key, error)
